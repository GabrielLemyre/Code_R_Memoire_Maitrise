...
void Filtering_2RegimesTVP_XExo (int *T, int *B,double *par, double *y, double *logLikSum, double *X_Exo, double *X_t, double *X_tlag, double *p11, double *p22) {
...
f1[0]=omega1+ A1*X_Exo[0];
f2[0]=omega2+ A2*X_Exo[0];
...
f1[t+1]=omega1+ A1*X_Exo[t+1];
f2[t+1]=omega2+ A2*X_Exo[t+1];
...
for (int t=*B; t < *T; t++) {
logLikSum[0]=logLikSum[0]+logLik[t];
	}
